...
st.set_page_config(page_title="Options Flow Summarizer (Pro Signals)", layout="wide")
...
def _aggregate(df: pd.DataFrame):
    ...
    by_strike = df.groupby(grp_cols).agg(
        premium_sum=("premium","sum"), oi=("open_interest","max"), vol=("volume","sum"),
        largest_trade=("size","max"), total_size=("size","sum"), under_px=("underlying_price","last"),
        iv_mean=("iv","mean")
    ).reset_index()
    ...
    inst100 = df.assign(_f=(df["size"]>=100).fillna(False)).groupby(grp_cols)["size"].apply(lambda s: float(s[s>=100].sum()) if (s>=100).any() else 0.0)
    inst500 = df.assign(_f=(df["size"]>=500).fillna(False)).groupby(grp_cols)["size"].apply(lambda s: float(s[s>=500].sum()) if (s>=500).any() else 0.0)
    totalsz = df.groupby(grp_cols)["size"].sum()
    by_strike = by_strike.merge(inst100.rename("inst_size_ge100"), on=grp_cols, how="left")
    by_strike = by_strike.merge(inst500.rename("inst_size_ge500"), on=grp_cols, how="left")
    by_strike = by_strike.merge(totalsz.rename("total_size"), on=grp_cols, how="left")
    by_strike["inst_share_ge100"] = by_strike.apply(lambda r: (r["inst_size_ge100"]/r["total_size"]) if r["total_size"] and r["total_size"]>0 else np.nan, axis=1)
    by_strike["inst_share_ge500"] = by_strike.apply(lambda r: (r["inst_size_ge500"]/r["total_size"]) if r["total_size"] and r["total_size"]>0 else np.nan, axis=1)
...
def hidden_signals(df: pd.DataFrame, by_strike: pd.DataFrame):
    ...
    near_term = by_strike[(by_strike["type"]=="Put") & (by_strike["strike_distance_pct"]>0)]
    near_term = near_term.merge(dtes, on="exp_date", how="left")
    ...
    # Far-dated calls (>120 DTE), often ITM/ATM
...
if uploaded is not None:
    ...
    st.subheader("Narrative Summary"); st.markdown(narrative)
